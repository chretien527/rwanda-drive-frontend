      ...
      {isMobileMenuOpen && (
        <div className='pointer-events-auto md:hidden mt-2 bg-[#0e1e38] text-white rounded-2xl p-5 border border-white/15 shadow-2xl space-y-4 animate-in fade-in slide-in-from-top-2 duration-200'>
          <div className='flex flex-col space-y-3 text-sm font-semibold text-slate-200'>
            <a href='#hero' onClick={() => setIsMobileMenuOpen(false)} className='hover:text-white py-1 transition-colors'>
              Overview
            </a>
            <a href='#how-it-works' onClick={() => setIsMobileMenuOpen(false)} className='hover:text-white py-1 transition-colors'>
              How It Works
            </a>
            <a href='#features' onClick={() => setIsMobileMenuOpen(false)} className='hover:text-white py-1 transition-colors'>
              Features
            </a>
            <a href='#entrusted-users' onClick={() => setIsMobileMenuOpen(false)} className='hover:text-white py-1 transition-colors'>
              Entrusted Drivers
            </a>
          </div>
          <div className='pt-3 border-t border-white/10 flex flex-col gap-2.5'>
            <Link href='/login' onClick={() => setIsMobileMenuOpen(false)} className='w-full py-2.5 text-center text-xs font-bold text-white bg-white/10 rounded-xl hover:bg-white/20 transition-all'>
              Sign In
            </Link>
            <Link href='/signup' onClick={() => setIsMobileMenuOpen(false)} className='w-full py-2.5 text-center text-xs font-bold text-[#0e1e38] bg-white rounded-xl shadow hover:bg-slate-100 transition-all'>
