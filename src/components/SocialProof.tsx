...
import { ShieldCheck, CheckCircle2, UserCheck, ArrowUpRight } from 'lucide-react';
...
export const SocialProof: React.FC = () => {
  const users = [
    {
      ...
      quote: 'I no longer carry paper Carte Jaune or physical insurance stickers. Roadside police verification takes less than 10 seconds at checkpoints.',
    ...
    },
    {
      ...
      quote: 'Managing inspection renewals and insurance certificates across 15 commercial transport vehicles used to be chaotic. The expiry radar alerts us automatically.',
    ...
    },
    {
      ...
      quote: 'The dynamic rotating QR code gives passenger trust and protects my National ID privacy during routine traffic stops on RN1 highway.',
  ...
  const partners = [
    ...
    { name: 'Rwanda Inter-City Transport Association', members: '450+ Buses' }
  ];

  return (
    <section id="entrusted-users" className="py-24 bg-white border-t border-slate-200 text-[#0e1e38] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {users.map((u, idx) => (
            <ScrollReveal
              ...
              duration={700}
            >
              <div className="h-full bg-slate-50 rounded-3xl p-8 border border-slate-200 hover:border-[#0e1e38] hover:shadow-xl transition-all duration-300 flex flex-col justify-between group">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3.5">
                    ...
                    />
                    <div>
                      <div className="font-black text-base text-[#0e1e38]">{u.name}</div>
                      <div className="text-xs text-slate-500 font-medium">{u.role}</div>
                      <div className="text-[11px] text-slate-400 font-semibold">{u.district}</div>
                    </div>
                  </div>
        ...
        {/* Partner & Fleet Ecosystem Banner (Darker #0e1e38 & Larger Dimensions as requested) */}
        <div className="bg-[#0e1e38] text-white rounded-3xl p-10 sm:p-14 shadow-2xl border border-white/10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            ...
            <div className="lg:col-span-7 grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
              {partners.map((p, i) => (
                <div key={i} className="p-5 rounded-2xl bg-white/10 border border-white/15 text-left space-y-1.5 backdrop-blur-xs">
                  <div className="text-base font-black text-white">{p.members}</div>
                  <div className="text-xs text-slate-300 font-medium leading-snug">{p.name}</div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-10 pt-8 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm">
            <div className="flex items-center gap-2.5 text-slate-300">
              ...
              <span>Ready to digitize your vehicle documents? Setup takes less than 2 minutes.</span>
            </div>
            <Link
              href="/signup"
